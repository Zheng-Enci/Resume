...
### 核心修改
1. 预览运行支持：完善配置以支持 preview 运行，便于快速验收与演示。
2. 后端地址动态配置：在配置文件中按运行模式（生产/非生产）自动切换后端项目地址，避免手工改配置。
3. 签到趋势并行加载：将个人分析页“按天串行加载”改为“全量并行加载”，显著减少跨多日查询的等待时间。

### 效果与价值
- Preview 运行可直接验证上线效果。
- 后端地址随环境自动切换，减少手工失误。
- 签到趋势在大时间跨度下加载速度明显提升，用户体验改善。
